...
InnoDB 스토리지 엔진은 필요한 레코드를 우선적으로 LRU의 데이터 페이지에서 찾는다.  
> LRU 리스트의 목적은 디스크로부터 읽어온 페이지를 오랫동안 InnoDB 버퍼 풀의 메모리에 유지해서 디스크 읽기를 최소화 하는 것이다.  

플러시 리스트는 데이터 변경이 가해진 데이터 페이지를 관리한다.  
프리 리스트는 비어 있는 페이지들의 목록이다.  

### 버퍼 풀과 리두 로그  
크게 설정할수록 쿼리의 성능이 향상된다.  
DB 성능 향상을 위해 데이터 캐시와 쓰기 버퍼링의 두 가지 용도로 사용된다.  
메모리 공간만 단순히 늘리는 것은 데이터 캐시 기능만 향상 시키는 것이다.  
버퍼링 기능까지 향상시키려면 InnoDB 버퍼 풀과 리두 로그의 관계를 이해해야 한다.  
...
버퍼 풀과 비교하여 리두 로그 파일의 크기가 너무 작다면 실제 쓰기 버퍼링을 위한 효과를 보기 어렵다.  
리두 로그 파일의 크기가 너무 크다면 급작스럽게 많은 더티 페이지를 한 번에 기록하므로 성능이 저하될 수 있다.  
> 리두 로그는 변경분만 가지므로 버퍼 풀과 비교하여 훨씬 작은 공간만 있으면 된다.  

## 언두 로그
InnoDB 스토리지 엔진은 DML로 변경되기 이전 버전의 데이터를 별도로 백업하고 이를 언두 로그(Undo Log)라고 한다.  
언두 로그는 **트랜잭션 보장**과 **격리 수준 보장**에 사용된다.  

데이터 변경을 시도하면 트랜잭션을 커밋하지 않아도 실제 데이터 파일 내용은 변경된다.  
변경되기 전의 값은 언두 영역에 백업되고 이는 롤백 및 높은 동시성을 제공하는 데 사용된다.  

## 체인지 버퍼
RDBMS에서 레코드가 Insert, Update 될 때는 인덱스를 업데이트 하는 작업이 필요하다.  
이는 랜덤하게 디스크를 읽는 작업이 필요하므로 인덱스가 많다면 많은 자원을 소모하게 된다.  
InnoDB는 변경할 인덱스 페이지가 버퍼 풀에 있으면 바로 업데이트를 수행하고, 디스크에 있다면 임시 공간에 저장해 두고 사용자에게 결과를 반환하는 형태로 성능을 향상시킨다.  
이때 사용하는 임시 메모리 공간을 **체인지 버퍼**(Change Buffer)라고 한다.  

## 어댑티브 해시 인덱스
InnoDB 스토리지 엔진에서 사용자가 자주 요청하는 데이터에 대해 자동으로 생성하는 인덱스이다.  
  
B-Tree 인덱스에서 값을 찾는 과정은 매우 빠르게 처리 된다고 인식하지만, DB 서버가 얼마나 많은 일을 하는지에 따라 성능이 느려질수도 있다.  
어댑티브 해시 인덱스는 B-Tree의 검색 시간을 줄여주기 위해 도입되었다.  
자주 읽히는 데이터 페이지의 키 값을 이용해 해시 인덱스를 만들고 이를 활용하여 레코드가 저장된 데이터 페이지를 즉시 찾아갈 수 있다.  

데이터 페이지를 버퍼 풀 내에서 접근하는 것을 더 빠르게 만드는 기능이기 때문에, 데이터를 디스크에서 읽어오는 경우가 빈번한 경우 도움이 되지 않는다.  
인덱스의 도움을 많이 받을 수록 테이블 삭제 또는 변경 작업에 불필요한 오버헤드가 발생하므로 서비스에 따라 적절히 선택해야 한다.  
